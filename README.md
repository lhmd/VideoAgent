...
```
Besides, create the environment of [Video-LLaVA](https://github.com/PKU-YuanGroup/Video-LLaVA) by running the following command. \
...
```
Copy ```path/to/bigai_ml/all_pretrained_weights/weights.zip``` to the directory of VideoAgent and run ```unzip weights.zip```, this will create two directory ```cache_dir```(the weights of VideoLLaVA) and ```tool_models```(the weights of all other models).
